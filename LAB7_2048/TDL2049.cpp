  ...
  httpsgithub.commoporgicTDL2048-Demo
 
  Computer Games and Intelligence (CGI) Lab, NCTU, Taiwan
  httpwww.aigames.nctu.edu.tw
...
#include cmath
  ...
  output streams
  to enable debugging (more output), just change the line to 'stdostream& debug = stdcout;'
 
stdostream& info = stdcout;
stdostream& error = stdcerr;
stdostream& debug = stdcout;
stdostream& debug = (new stdofstream);
...
class board {
    ...
    private
        ...
        struct lookup {
            ...
            void init(int r) {
                ...
                score = mvleft(R); stdreverse(R, R + 4);
    ...
    public
        ...
        int move_left() {
            ...
            uint64_t prev = raw;
            int score = 0;
            lookupfind(fetch(0)).move_left(move, score, 0);
            lookupfind(fetch(1)).move_left(move, score, 1);
            lookupfind(fetch(2)).move_left(move, score, 2);
            lookupfind(fetch(3)).move_left(move, score, 3);
            raw = move;
        ...
        int move_right() {
            ...
            uint64_t prev = raw;
            int score = 0;
            lookupfind(fetch(0)).move_right(move, score, 0);
            lookupfind(fetch(1)).move_right(move, score, 1);
            lookupfind(fetch(2)).move_right(move, score, 2);
            lookupfind(fetch(3)).move_right(move, score, 3);
            raw = move;
            return (move != prev)  score  -1;
        }
        int move_up() {
    ...
    public
        ...
        friend stdostream& operator (stdostream& out, const board& b) {
            char buff[32];
            out  +------------------------+  stdendl;
...
class feature {
    ...
    public  should be implemented
        ...
        virtual stdstring name() const = 0;
    ...
    public
        ...
        virtual void dump(const board& b, stdostream& out = info) const {
            out  b  estimate =   estimate(b)  stdendl;
        }

        friend stdostream& operator (stdostream& out, const feature& w) {
            stdstring name = w.name();
            int len = name.length();
            out.write(reinterpret_castchar(&len), sizeof(int));
            out.write(name.c_str(), len);
            float weight = w.weight;
            size_t size = w.size();
            out.write(reinterpret_castchar(&size), sizeof(size_t));
            out.write(reinterpret_castchar(weight), sizeof(float)  size);
            return out;
        }

        friend stdistream& operator (stdistream& in, feature& w) {
            stdstring name;
            int len = 0;
            in.read(reinterpret_castchar(&len), sizeof(int));
            name.resize(len);
            in.read(&name[0], len);
            if (name != w.name()) {
                error  unexpected feature   name   (  w.name()   is expected)  stdendl;
                stdexit(1);
            }
            float weight = w.weight;
            size_t size;
            in.read(reinterpret_castchar(&size), sizeof(size_t));
            if (size != w.size()) {
                error  unexpected feature size   size  for   w.name();
                error   (  w.size()   is expected)  stdendl;
                stdexit(1);
            }
            in.read(reinterpret_castchar(weight), sizeof(float)  size);
            if (!in) {
                error  unexpected end of binary  stdendl;
                stdexit(1);
            }
            return in;
        }
    ...
    protected
        static float alloc(size_t num) {
            ...
            try {
                total += num;
                if (total  limit) throw stdbad_alloc();
                return new float[num]();
            } catch (stdbad_alloc&) {
                error  memory limit exceeded  stdendl;
                stdexit(-1);
            }
            return nullptr;
        }
  ...
  the pattern feature
  including isomorphic (rotatemirror)
...
class pattern  public feature {
    public
        pattern(const stdvectorint& p, int iso = 8)  feature(1  (p.size()  4)), iso_last(iso) {
            if (p.empty()) {
                error  no pattern defined  stdendl;
                stdexit(1);
            }
    ...
    public
        ...
        virtual stdstring name() const {
            return stdto_string(isomorphic[0].size()) + -tuple pattern  + nameof(isomorphic[0]);
        }
    ...
    public
        ...
        void dump(const board& b, stdostream& out = info) const {
            for (int i = 0; i  iso_last; i++) {
                out  #  i    nameof(isomorphic[i])  (;
                size_t index = indexof(isomorphic[i], b);
                for (size_t i = 0; i  isomorphic[i].size(); i++) {
                    out  stdhex  ((index  (4  i)) & 0x0f);
                }
                out  stddec  ) =   operator[](index)  stdendl;
            }
        }

    protected

        size_t indexof(const stdvectorint& patt, const board& b) const {
            size_t index = 0;
        ...
        stdstring nameof(const stdvectorint& patt) const {
            stdstringstream ss;
            ss  stdhex;
            stdcopy(patt.cbegin(), patt.cend(), stdostream_iteratorint(ss, ));
            return ss.str();
        }

        stdarraystdvectorint, 8 isomorphic;
        int iso_last;
    };
...
class state {
    public
        state(int opcode = -1)
             opcode(opcode), score(-1), esti(-stdnumeric_limitsfloatmax()) {}
        state(const board& b, int opcode = -1)
             opcode(opcode), score(-1), esti(-stdnumeric_limitsfloatmax()) { assign(b); }
    ...
    public
        ...
        bool operator  (const state& s) const {
            if (before != s.before) throw stdinvalid_argument(stateoperator);
            return esti  s.esti;
        }
    ...
    public
        ...
        bool is_valid() const {
            if (stdisnan(esti)) {
                error  numeric exception  stdendl;
                stdexit(1);
            }
        ...
        friend stdostream& operator (stdostream& out, const state& st) {
            out  moving   st.name()  , reward =   st.score;
            if (st.is_valid()) {
                out  , value =   st.esti  stdendl  st.after;
            } else {
                out   (invalid)  stdendl;
            }
            return out;
...
class learning {
    public
        ...
        ~learning() {}
          ...
          note that feats is stdvectorfeature,
        ...
        float evaluate(const board& b) const {
            ...
            for(int i=0;i16;++i) {
                if (b.at(i) == 0) {
                    ...
                    board b2(b);
                    b2.set(i, 1);
                    result += 0.9  estimate(b2);
                     try 4
                    board b4(b);
                    b4.set(i, 2);
                    result += 0.1  estimate(b4);

                }
            }
            debug  value    result  cnt_empty  stdendl;
            return cnt_empty  result  cnt_empty  0;
        }
        ...
        state select_best_move(const board& b) const {
            ...
            for (state move = after; move != after + 4; move++) {
                if (move-assign(b)) {
                    ...
                    move-set_value(-stdnumeric_limitsfloatmax());
                }
                debug  test   move;
            }
            return best;
        ...
        void update_episode(stdvectorstate& path, float alpha = 0.1) const {
            float exact = 0;
            for (path.pop_back()  terminal state ; path.size(); path.pop_back()) {
                state& move = path.back();
                float error = exact - (estimate(move.before_state()) - move.reward());
                debug  update error =   error   for before state  stdendl  move.before_state();
        ...
        void make_statistic(size_t n, const board& b, int score, int unit = 1000) {
            stdofstream file;
            file.open(score, stdiosout  stdfstreamapp);
            scores.push_back(score);
            maxtile.push_back(0);
            for (int i = 0; i  16; i++) {
                maxtile.back() = stdmax(maxtile.back(), b.at(i));
            }

            if (n % unit == 0) {  show the training process
                if (scores.size() != size_t(unit)  maxtile.size() != size_t(unit)) {
                    error  wrong statistic size for show statistics  stdendl;
                    stdexit(2);
                }
                int sum = stdaccumulate(scores.begin(), scores.end(), 0);
                int max = stdmax_element(scores.begin(), scores.end());
                int stat[16] = { 0 };
                for (int i = 0; i  16; i++) {
                    stat[i] = stdcount(maxtile.begin(), maxtile.end(), i);
                }
                ...
                for (int t = 1, c = 0; c  unit; c += stat[t++]) {
                    ...
                    int accu = stdaccumulate(stat + t, stat + 16, 0);
                    info  t  ((1  t) & -2u)  t  (accu  coef)  %;
                    info  t(  (stat[t]  coef)  %)  stdendl;
                    if (((1  t) & -2u) == 2048){
                        file  (accu  coef)     mean  stdendl;
                        flag_2048 = 1;
                    }
                }
                if(!flag_2048)
        ...
        void load(const stdstring& path) {
            stdifstream in;
            in.open(path.c_str(), stdiosin  stdiosbinary);
            if (in.is_open()) {
                size_t size;
                in.read(reinterpret_castchar(&size), sizeof(size));
                if (size != feats.size()) {
                    error  unexpected feature count   size   (  feats.size()   is expected)  stdendl;
                    stdexit(1);
                }
                for (feature feat  feats) {
                    in  feat;
                    info  feat-name()   is loaded from   path  stdendl;
                }
                in.close();
            }
        }
        ...
        void save(const stdstring& path) {
            stdofstream out;
            out.open(path.c_str(), stdiosout  stdiosbinary  stdiostrunc);
            if (out.is_open()) {
                size_t size = feats.size();
                out.write(reinterpret_castchar(&size), sizeof(size));
                for (feature feat  feats) {
                    out  feat;
                    info  feat-name()   is saved to   path  stdendl;
                }
                out.flush();
                out.close();
            }
    ...
    private
        stdvectorfeature feats;
        stdvectorint scores;
        stdvectorint maxtile;
};

int main(int argc, const char argv[]) {
    info  TDL2048-Demo  stdendl;
    learning tdl;
    ...
    info  alpha =   alpha  stdendl;
    info  total =   total  stdendl;
    info  seed =   seed  stdendl;
    stdsrand(seed);
    ...
    for (size_t n = 1; n = total; n++) {
        ...
        int score = 0;
         ...
         play an episode
        debug  begin episode  stdendl;
        b.init();
        while (true) {
            debug  state  stdendl  b;
            state best = tdl.select_best_move(b);
            path.push_back(best);

            if (best.is_valid()) {
                debug  best   best;
                debug  best before  stdendl  best.before_state();
                debug  best after  stdendl  best.after_state();
                score += best.reward();
                b = best.after_state();
                b.popup();
            } else {
